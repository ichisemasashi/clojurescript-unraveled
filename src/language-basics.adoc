...
このような状況に陥ったことは一度や二度ではないと思います。 「ビジネスロジック」のために（インターフェイスなどを使用して）優れた抽象化を定義した後、全く制御できない別のモジュールを扱う必要性に気付き、おそらくアダプタやプロキシなど、大量の追加の複雑さを含むアプローチを作成しようと考えていたのではないでしょうか。


動的言語の中には、「モンキーパッチ」が可能なものがあります。これは、クラスがオープンで、どのメソッドもいつでも定義・再定義が可能な言語です。また、この手法が非常に悪いものであることはよく知られています。

サードパーティのライブラリをインポートする際に、自分が使っているメソッドを黙って上書きできるような言語は信用できませんし、このような状況では一貫した動作は期待できません。

このような症状は、一般的に「式の問題」と呼ばれています。
詳しくは http://en.wikipedia.org/wiki/Expression_problem を参照してください。


==== Protocols

「インターフェース」を定義するための _ClojureScript_ のプリミティブをプロトコルと呼びます。プロトコルは名前と関数のセットで構成されます。すべての関数は、JavaScriptの `this` やPythonの `self` に相当する少なくとも1つの引数を持ちます。

プロトコルは型に基づいた多相性を提供し、ディスパッチは常に第一引数（前述の通り、JavaScriptの `this` に相当）によって行われます。
...
注：「I」という接頭辞は、プロトコルと型の分離を指定するためによく使われます。Clojureコミュニティでは、"I"という接頭辞をどのように使用すべきかについて様々な意見があります。我々の意見では、名前の衝突や起こりうる混乱を避けるための許容できる解決策です。しかし、接頭辞を使用しないことは、悪い習慣とはみなされません。

ユーザーの視点から見ると、プロトコル関数は、プロトコルが定義されている名前空間で定義された単なるプレーンな関数です。これにより、同じ型に実装された異なるプロトコルの間で、関数名の衝突を避けるための簡単でシンプルなアプローチが可能になります。

以下にその例を示す。 "反転" できるデータのために、 `IInvertible` というプロトコルを作成してみましょう。 このプロトコルは `invert` という名前のメソッドを1つ持ちます。

[source, clojure]
----
(defprotocol IInvertible
  "「反転可能( 'invertible' )」なデータ型のためのプロトコル"
...
プロトコルの大きな強みの一つは、既存の、あるいはサードパーティの型を拡張できることです。この操作はさまざまな方法で行うことができます。

ほとんどの場合、 *extend-protocol* または *extend-type* マクロを使用することになります。これは `extend-type` 構文がどのように見えるかを示しています。
...
*extend-type* を使うと、1つの式で1つの型を異なるプロトコルで拡張していることがわかります。

先ほど定義した「IInvertible」プロトコルで遊んでみましょう。
...
文字列のプロトタイプを拡張するために、 `js/String` ではなく、特別なシンボル *string* が使用されていることに気づくかもしれません。これは、javascriptの組み込み型は特別な扱いを受けており、 `string` を `js/String` に置き換えると、コンパイラがその旨の警告を発するためです。

したがって、プロトコルをjavascriptのプリミティブな型に拡張したい場合には、 `js/Number` ,  `js/String` ,  `js/Object` ,  `js/Array` , `js/Boolean` , `js/Function` の代わりに、それぞれの特別なシンボルを使用する必要があります。 `number` ,  `string` , `object` , `array` , `boolean` , `function` です。

さて、いよいよプロトコルの実装を試してみましょう。
...
これに対し、 *extend-protocol* はその逆で、プロトコルが与えられれば、複数の型に対する実装を追加します。このような構文になっています。
...
このように、先ほどの例はこの方法でも同じように書けたはずです。
...
===== Participate in ClojureScript abstractions (ClojureScriptの抽象化に参加する)

ClojureScript自体は、プロトコルとして定義された抽象化の上に構築されています。 _ClojureScript_ 言語自体のほとんどすべての動作は、サードパーティのライブラリに適応することができます。実際の例を見てみましょう。

前のセクションでは、さまざまな種類の組み込みコレクションについて説明しました。この例では、 *set* を使用します。このコードのスニペットをご覧ください。
...
何が起こったのでしょうか？この場合、_set_ 型は _ClojureScript_ 内部の `IFn` プロトコルを実装しており、関数やcallableを抽象化して表しています。このため、フィルタのcallableな述語のように使うことができます。

しかし、文字列のコレクションをフィルタリングするための述語関数として、正規表現を使いたい場合はどうなるでしょうか。
...
例外が発生するのは、`RegExp` 型が `IFn` プロトコルを実装していないため、 callable のように振る舞うことができないからですが、これは簡単に修正することができます。
...
これを分析してみましょう。私たちは `js/RegExp` 型を拡張して、`IFn` プロトコルの `invoke` 関数を実装しています。正規表現 `a` をあたかも関数のように呼び出すには，関数のオブジェクトとパターンを指定して `re-find` 関数を呼び出します．

これで、フィルタ操作の述語として正規表現のインスタンスを使うことができるようになります。
...
_ClojureScript_ には、実行時のイントロスペクションを可能にする便利な関数が付属しています: `satisfies?` です。この関数の目的は、あるオブジェクト(ある型のインスタンス)が具象プロトコルを満たしているかどうかを実行時に判断することです。

前述の例では、 `set` インスタンスが *IFn* プロトコルを満たしているかどうかをチェックすると、 `true` を返すはずです。
...
以前、ポリモーフィズムの非常に一般的な使用例である「型によるディスパッチ」を解決するプロトコルについて説明しました。しかし、状況によっては、プロトコルのアプローチには限界があります。ここで、 *multimethods* が救いの手を差し伸べます。

これらの *multimethods* は、型によるディスパッチだけではなく、複数の引数の型によるディスパッチや値によるディスパッチも提供します。また、その場限りの階層を定義することも可能です。また、マルチメソッドはプロトコルと同様に「オープンシステム」であり、ユーザーやサードパーティが新しい型のためにマルチメソッドを拡張することができます。

*multimethods* の基本的な構造は、 `defmulti` 形式と `defmethod` 形式です。 `defmulti` 形式は初期ディスパッチ関数を持つマルチメソッドを作成するために使用されます。これは、そのモデルとなるものです。


[source, clojure]
----
(defmulti say-hello
  "デフォルトの言語を `:en` とし、言語キーに応じて挨拶メッセージを返す多相性関数"
  (fn [param] (:locale param))
  :default :en)
----

`defmulti` フォーム内で定義された無名関数はディスパッチ関数です。この関数は、 `say-hello` 関数が呼び出されるたびに呼び出され、ディスパッチに使用される何らかのマーカーオブジェクトを返す必要があります。この例では、第一引数の `:locale` キーの内容を返しています。

そして最後に、実装を追加する必要があります。これには `defmethod` という形式を使います。
...
つまり、`:locale` とオプションで `:name` キーを含むハッシュマップに対してこの関数を実行した場合、マルチメソッドはまずディスパッチ関数を呼び出してディスパッチ値を決定し、次にその値の実装を探します。実装が見つかった場合、ディスパッチャはそれを実行します。それ以外の場合は、（指定されていれば）デフォルトの実装を検索し、それを実行します。
...
デフォルトの実装が指定されていない場合は、ある値がそのマルチメソッドの実装を持っていないことを通知する例外が発生します。


==== Hierarchies

階層は、_ClojureScript_の方法で、あなたのドメインが必要とする関係を構築することができます。階層は、シンボル、キーワード、型など、名前の付いたオブジェクト間の関係で定義されます。

階層は、必要に応じて、グローバルにもローカルにも定義できます。マルチメソッドのように、階層は単一のネームスペースに限定されません。階層は、定義されているネームスペースだけでなく、どのネームスペースからでも拡張できます。

グローバルな名前空間はより限定的ですが、それには理由があります。名前空間でないキーワードやシンボルは、グローバル階層では使用できません。この動作により、複数のサードパーティライブラリが同じシンボルを異なるセマンティクスで使用するという予期せぬ事態を防ぐことができます。


===== Defining a hierarchy

階層関係は、 `derive` 関数を使って確立する必要があります。
...
名前付きのキーワードの関係を定義しました。この場合、`::circle` は `::shape` の子であり、`::box` は `::shape` の子でもあります。

ヒント: `::circle` キーワードの構文は、`:current.ns/circle` の省略形です。そのため、REPLで実行する場合、 `::circle` は `:cljs.user/circle` として評価されます。


===== Hierarchies and introspection

_ClojureScript_には、グローバルまたはローカルに定義された階層を、実行時にイントロスペクションできるようにする関数の小さなツールセットが付属しています。このツールセットは3つの関数で構成されています。 `isa?` ,  `ancestors` ,  `descendants`  です。

前述の例で定義された階層を使って、どのように使用するかの例を見てみましょう。
...
先に述べたように、 _ClojureScript_ ではローカルな階層を定義することもできます。これは `make-hierarchy` という関数を使って行うことができます。ここでは、ローカルな階層を使って、先ほどの例を再現してみましょう。
...
これで、ローカルに定義された階層で、同じイントロスペクション関数を使用することができます。
...
ご覧のとおり、ローカル階層では、通常の（名前空間修飾されていない）キーワードを使用することができ、ローカル階層のパラメータを渡さずに `isa?` を実行すると、予想通り `false` を返します。


===== Hierarchies in multimethods

階層構造の大きな利点の一つは、マルチメソッドとの相性が非常に良いことです。 これは、マルチメソッドがデフォルトでディスパッチの最後のステップに `isa?` 関数を使用するためです。

この意味を明確に理解するために、例を見てみましょう。まず、 `defmulti` という形式でマルチメソッドを定義します。

[source, clojure]
----
(defmulti stringify-shape
  "shapeキーワードの人間が読める表現を表示する関数。"
  identity
  :hierarchy #'h)
----

キーワードパラメータ `:hierarchy` を使って、使用したい階層をマルチメソッドに示します。指定されない場合は、グローバルな階層が使用されます。


次に、`defmethod` フォームを使用して、マルチメソッドの実装を定義します。
...
では、その関数を:boxで実行するとどうなるか見てみましょう。
...
これで、すべてが期待通りに動作するようになりました。マルチメソッドは、与えられたパラメータに対して適合する実装を実行します。次に、同じ関数を実行しますが、適合するディスパッチ値を持たない `:circle` キーワードをパラメータとして使用した場合にどうなるかを見てみましょう。
...
マルチメソッドは提供された階層を使って自動的に解決します。そして、`:circle` は `:shape` の子孫なので、`:shape` の実装が実行されます。

最後に、階層に含まれていないキーワードを与えた場合は、`:default`の実装が実行されます。
...
これまで私たちは、データを表現するのに、マップ、セット、リスト、ベクターなどを使ってきました。そして、ほとんどの場合、これは本当に素晴らしいアプローチだと思います。しかし、時には独自の型を定義する必要があります。本書では、それらを *データ型* と呼びます。

データ型には次のような特徴があります。

* 名前付きまたは匿名の、ホストに支持された固有の型。
* プロトコルを実装する機能（インライン）。
* フィールドやクロージャを使った明示的な構造宣言。
* マップのような動作（レコードを介して、以下を参照）。


==== Deftype

_ClojureScript_ で独自の型を作成するための最も低レベルな構造は、 `deftype` マクロです。デモとして、 `User` という型を定義してみましょう。
...
型が定義されると、 `User` のインスタンスを作成することができます。以下の例では、 `User` の後の `.` はコンストラクタを呼び出していることを示しています。
...
そのフィールドには、プレフィックスのドット表記でアクセスできます。
...
`deftype` (と、後で説明する `defrecord` ) で定義された型は、現在の名前空間に関連付けられた、ホストバックされたクラスのようなオブジェクトを作成します。便宜上、 _ClojureScript_ では `->User` というコンストラクタ関数も定義しており、`:require` ディレクティブでインポートすることができます。

個人的にはこのような関数は好きではありませんし、もっとわかりやすい名前で独自のコンストラクタを定義したいと思っています。
...
このコードでは、`->User`の代わりにこれを使用しています。


==== Defrecord

レコードは、 _ClojureScript_ で型を定義するための、少し高いレベルの抽象化であり、好ましい方法です。

ご存知のように、 _ClojureScript_ はマップなどのプレーンなデータ型を使う傾向にありますが、ほとんどの場合、アプリケーションのエンティティを表すために名前付きの型が必要です。ここで登場するのがレコードです。

レコードはマッププロトコルを実装したデータ型で、他のマップと同様に使用することができます。 また、レコードは適切な型でもあるので、プロトコルによる型ベースのポリモーフィズムをサポートしています。

まとめると、レコードを使うことで、異なる抽象度でプレイできるマップという、両方の世界の良いところを手に入れることができます。

それでは早速、レコードを使って `User` という型を定義してみましょう。
...
これは、 `deftype` 構文にとてもよく似ています。
実際には、型を定義するための低レベルのプリミティブとして、舞台裏で `deftype` を使用しています。

では、フィールドにアクセスするための生の型との違いを見てみましょう。
...
先に述べたように、レコードはマップであり、そのように動作します。
...
また、マップと同様に、最初に定義されていない追加フィールドをサポートしています。
...
見ての通り、 `assoc` 関数は期待通りに動作し、同じ型で新しいキーバリューペアを持つ新しいインスタンスを返します。しかし、 `dissoc` には注意が必要です! レコードでの動作は、マップの場合とは少し異なります。dissocするフィールドがオプションフィールドの場合は新しいレコードを返しますが、必須フィールドを解散した場合はプレーンなマップを返します。

マップとのもうひとつの違いは、レコードが関数のようには動作しないことです。
...
便宜上、`defrecord` マクロは `deftype` と同様に `->User` 関数を公開しており、さらに `map->User` コンストラクタ関数も公開しています。このコンストラクタについては、 `deftype` で定義されたものと同じ意見で、他のものを使うのではなく、自分で定義することをお勧めします。しかし、コンストラクタが存在する以上、それがどのように使われるのかを見てみましょう。
...
これまで見てきた型定義プリミティブは、どちらもプロトコルのインライン実装を可能にしています（前のセクションで説明しました）。ここでは、例としてプロトコルを定義してみましょう。
...
ここで、抽象化のためのインライン実装の型を定義することができます。ここでは、 `IUser` とします。
...
`reify` マクロは、型を事前に定義せずにオブジェクトを作成するために使用できる _アドホックコンストラクタ_ です。 プロトコルの実装は `deftype` や `defrecord` と同じように提供されますが、対照的に `reify` はアクセス可能なフィールドを持ちません。

このようにして、`IUser`の抽象化と相性の良いユーザー型のインスタンスをエミュレートすることができます。
...
`specify!` は `reify` に代わる高度な機能で、既存の JavaScript オブジェクトにプロトコルの実装を追加することができます。 これは、JavaScriptライブラリのコンポーネントにプロトコルを移植したい場合に便利です。
...
`specify` は `specify!` の不変バージョンで、`ICloneable` を実装した不変でコピー可能な値（ClojureScript のコレクションなど）に使用することができます。
...
=== Host interoperability (ホストの相互運用性)

_ClojureScript_ は、兄のClojureと同様に、「ゲスト」言語として設計されています。つまり、 _ClojureScript_ ではJavaScript、 _Clojure_ ではJVMといった、既存のエコシステムの上でうまく機能するように設計されています。


==== The types

_ClojureScript_ は、皆さんが予想しているのとは異なり、プラットフォームが提供するすべての型を利用しようとします。以下は、 _ClojureScript_ が基礎となるプラットフォームから継承・再利用するものの（おそらく不完全な）リストです。

* _ClojureScript_ の文字列は、JavaScript の *Strings* です。
* _ClojureScript_ の数字は、JavaScript の *Numbers* です。
* _ClojureScript_ `nil` は、JavaScript の *null* です。
* _ClojureScript_ 正規表現は、JavaScript の `RegExp` インスタンスです。
* _ClojureScript_ は解釈されず、常に JavaScript にコンパイルされます。
* _ClojureScript_ は同じセマンティクスを持つプラットフォームの API を簡単に呼び出すことができます。
* _ClojureScript_ のデータ型は、内部的には JavaScript のオブジェクトにコンパイルされます。

その上で、 _ClojureScript_ はプラットフォームには存在しない独自の抽象化と型を構築します。例えば、ベクターやマップ、セットなど、この章の前のセクションで説明されているものです。


==== Interacting with platform types

_ClojureScript_ には、オブジェクトのメソッドを呼び出したり、新しいインスタンスを作成したり、オブジェクトのプロパティにアクセスしたりといった、プラットフォーム型との対話を可能にする特別なフォームの小さなセットが付属しています。


===== Access to the platform

_ClojureScript_は、 `js/` という特別な名前空間を通して、プラットフォーム環境全体にアクセスするための特別な構文を持っています。これは、JavaScriptの組み込み関数である `parseInt` を実行するための式の例です。
...
_ClojureScript_には、インスタンスを作成する2つの方法があります。
...
最後のものは、インスタンスを作成するのに推奨される方法です。この2つの形式の間に実際の違いがあることは知られていませんが、ClojureScriptコミュニティでは、最後のものが最もよく使われています。


===== Invoke instance methods

あるオブジェクトのインスタンスのメソッドを呼び出すには、JavaScriptで行われている方法（例： `obj.method()` ）とは異なり、Lisp言語の他の標準的な関数と同じようにメソッド名が最初に来ますが、関数名が特別な形式 `.` で始まるという、少し変わった点があります。

では、正規表現インスタンスの `.test()` メソッドをどのように呼び出すかを見てみましょう。
...
JavaScriptオブジェクトのインスタンスメソッドを呼び出すことができます。最初の例はこれまで見てきたパターンに従ったもので、最後の例はショートカットです。
...
オブジェクトのプロパティへのアクセスは，メソッドを呼び出すのと非常によく似ています。違いは、 `.` を使う代わりに、 `.-` を使うことです。例を見てみましょう。
...
接頭辞が `js/` のシンボルには、ドットを含めることができ、入れ子になったプロパティアクセスを表します。次の式は，どちらも同じ関数を呼び出すものです。
...
また、以下の表現はどちらも同じプロパティにアクセスしています。
...
_ClojureScript_ には、プレーンなJavaScriptオブジェクトを作成するための様々な方法があり、それぞれが独自の目的を持っています。基本的なものは `js-obj` 関数です。この関数は可変数のキーと値のペアを受け取り、JavaScriptオブジェクトを返します。
...
この戻り値は、プレーンなJavaScriptオブジェクトを受け入れる何らかのサードパーティのライブラリに渡すことができますが、この関数の戻り値の実際の表現を観察することができます。これは実際には同じことをするための別のフォームです。

リーダーマクロ `#js` を使うことは、ClojureScript のマップやベクターの前にそれを置くことであり、その結果はプレーンな JavaScript に変換されます。
...
これをプレーンなJavaScriptに変換すると、こんな感じになります。
...
前のセクションで説明したように、 `.-` 構文を使って、プレーンなオブジェクトのプロパティにアクセスすることもできます。
...
また，JavaScriptのオブジェクトは可変型なので，`set!`関数を使って，あるプロパティに新しい値を設定することができます。
...
先に説明したフォームの不都合な点は、再帰的な変換を行わないため、入れ子になったオブジェクトがある場合、入れ子になったオブジェクトは変換されないことです。 Clojurescriptのマップを使った例と、JavaScriptのオブジェクトを使った同様の例を考えてみましょう。
...
このユースケースを解決するために、_ClojureScript_ には `clj->js` と `js->clj` という関数が用意されており、Clojure のコレクションタイプを JavaScript に変換して戻します。ClojureScriptへの変換では、`:country`キーワードが文字列に変更されることに注意してください。
...
配列の場合には，特殊な関数 `into-array` があり，期待通りの動作をします．
...
前の例では、既存の_ClojureScript_コレクションから配列を作成する方法を見ました。しかし、配列を作るための関数はもう一つあります。それが `make-array` です。
...
_ClojureScript_ では、配列はシーケンスの抽象化にも対応しており、配列を反復処理したり、 `count` 関数で要素数を取得することができます。
...
JavaScriptプラットフォームの配列は、変更可能なコレクション型であるため、具体的なインデックスにアクセスして、その位置に値を設定することができます。
...
あるいは、その値を取得するために、インデックス付きの方法でアクセスします。
...
JavaScriptでは、配列のインデックスアクセスは、オブジェクトのプロパティにアクセスすることと同じなので、プレーンなオブジェクトを操作するのにも同じ関数を使うことができます。
...
ClojureScriptの基本的な考え方の1つに「不変性」があることを学びました。ClojureScriptでは、`Date`のようなJSホストに存在する可変型を除き、スカラ値もコレクションも不変です。

不変性には多くの優れた特性がありますが、時間とともに変化する値をモデル化する必要に迫られることがあります。その場でデータ構造を変更できない場合、どのようにしてこれを実現するのでしょうか。


==== Vars

Varsは名前空間内で自由に再定義できますが、 *いつ* 変更されたのかを知る方法はありません。他の名前空間のvarsを再定義できないという、ちょっとした制限があります。また、状態を変更している場合、それがいつ発生したのかを知りたいと思うでしょう。


==== Atoms

ClojureScriptでは、自由に変更できる値を含むオブジェクトである `Atom` 型が用意されています。アトム型は、値を変更できるだけでなく、アトムに着脱可能なウォッチャー関数による観察や、アトムに含まれる値が常に有効であることを保証する検証(validation)もサポートしています。

Ciriという人に対応するIDをモデル化する場合、Ciriのデータを含む不変の値をアトムで包むことができます。なお、アトムの値を得るには、 `deref` 関数やその省略形である `@` 記法を使います。
...
アトムの値を関数で変化させるには，アトムの`swap!`関数を使います．Ciriの誕生日は今日なので、彼女の年齢カウントを増やそう。
...
関数 `reset!` は、アトムに含まれる値を新しい値に置き換えます。
...
アトムのウォッチャー関数を追加したり削除したりすることができます。アトムの値が `swap!` や `reset!` によって変更されるたびに、アトムのすべてのウォッチャー関数が呼び出されます。ウォッチャーは `add-watch` 関数で追加されます。各ウォッチャーにはキー（例では `:logger` ）が関連付けられており、後でアトムからウォッチを削除するのに使用されることに注意してください。
...
volatileは、atomと同様に、変化させることのできる値を含むオブジェクトです。しかし、アトムのような観測や検証の機能はありません。そのため、パフォーマンスが若干高く、観測や検証を必要としないステートフルな関数の内部で使用するのに適した可変型コンテナとなっています。

そのAPIはatomとよく似ています。また、`vswap!`と`vreset!`でそれぞれスワップとリセットをサポートしています。
...
atomとのもう一つの違いは、volatileのコンストラクタでは最後にbangを使うことです。volatileは `volatile!` で、原子は `atom` で作ります。
